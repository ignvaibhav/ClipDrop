...
### 1.1 Product Summary
Island & Ferry is an open-source, two-part system that lets users download videos from YouTube (and later other platforms) directly from their browser — with no terminal, no technical knowledge, and no friction. It consists of a **Ferry browser extension** and an **Island desktop companion app**, connected via a local server.
...
- **DMCA posture:** Island & Ferry is a local tool — equivalent to youtube-dl, yt-dlp, JDownloader. No hosted content, no proxying.
