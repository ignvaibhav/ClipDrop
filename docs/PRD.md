# ClipDrop — Product Requirements Document (PRD)
...
### 1.1 Product Summary
ClipDrop is an open-source, two-part system that lets users download videos from YouTube (and later other platforms) directly from their browser — with no terminal, no technical knowledge, and no friction. It consists of a **browser extension** and a **desktop companion app**, connected via a local server.
...
### 1.3 Solution
A lightweight browser extension that injects a native-feeling download button on video pages. The extension captures user intent (format, quality, optional clip range) and hands off to a locally-running Tauri desktop app that silently executes `yt-dlp` in the background. The user never leaves the browser.
...
│   YouTube Page                                              │
│   ┌─────────────────────────────────────────────────────┐  │
│   │  [Like] [Dislike] [Share] [ClipDrop ↓]  ← injected  │  │
│   └─────────────────────────────────────────────────────┘  │
│                          │                                  │
...
1. User downloads ClipDrop desktop app (installer for their OS)
2. App installs, registers for auto-start, launches in system tray
3. User installs ClipDrop browser extension from Chrome/Firefox store
4. Done — no configuration required

### 6.2 Download a Video
1. User visits a YouTube video page
2. Extension detects the page, injects "↓ ClipDrop" button in action bar
3. User clicks the button
4. Extension pings `/health` — if app not running, shows "Open ClipDrop app" prompt
...
- **DMCA posture:** ClipDrop is a local tool — equivalent to youtube-dl, yt-dlp, JDownloader. No hosted content, no proxying.
- **Store listing language:** Describe as *"media companion launcher for ClipDrop desktop"* — avoid "download", "rip", "save" in store descriptions
...
*This document is the single source of truth for ClipDrop v0.1 scope and architecture. Update it as decisions evolve.*