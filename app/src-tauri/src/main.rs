...
//! ClipDrop desktop companion — system tray app with local API server.
//!
...
fn open_settings_window(app: &AppHandle) -> anyhow::Result<()> {
...
fn open_downloads_folder(_config: &AppConfig) -> anyhow::Result<()> {
    // Use blocking approach since this is called from sync tray handler.
    // TODO: use _config.download_dir() once we add async support here.
    let path = config::default_download_dir();
...
fn main() {
    ...
    info!(version = server::VERSION, "starting ClipDrop desktop");
    ...
    tauri::async_runtime::spawn(async move {
        if let Err(err) = server::run(api_queue, api_config, API_PORT).await {
            error!(error = %err, "ClipDrop API server crashed");
        }
    });
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let _ = TrayIconBuilder::with_id("clipdrop")
                .menu(&menu)
                .on_menu_event({
                    ...
                    move |app, event| match event.id.as_ref() {
                        ...
                        "open_downloads" => {
                            if let Err(err) = open_downloads_folder(&config) {
        ...
        .expect("error while running ClipDrop Tauri app");
}
