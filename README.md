...
ClipDrop is a local-first YouTube downloader composed of:

- `extension/`: Chrome MV3 extension that injects a `ClipDrop` button on YouTube watch pages.
- `app/src-tauri/`: Tauri desktop companion that runs a local API (`127.0.0.1:49152`) and executes `yt-dlp`.
...
4. Select `extension/`
5. Open a YouTube watch page and click the injected `ClipDrop` button
