...
- A browser extension (Ferry) injects a `Ferry` button on YouTube watch pages.
- A local Tauri desktop app (Island) exposes an HTTP/WebSocket API on `127.0.0.1:49152`.
...
- MV3 extension (Ferry) scaffolded.
- YouTube content script injects `Ferry` button.
