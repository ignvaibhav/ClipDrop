...
- Product shape:
  - `extension/` = Chrome MV3 extension injecting ClipDrop action into YouTube watch pages
  - `app/src-tauri/` = local Tauri desktop companion exposing API on `127.0.0.1:49152`
- Core product intent:
  - User clicks injected ClipDrop button on YouTube
  - Extension fetches available formats from local backend
...
- Accepts path and opens containing folder using:
  ...
  - `open` (macOS)
  - `xdg-open` (Linux)
- Used by extension notification click behavior

### 4.4 Download path settings support

- Added persisted desktop settings in Tauri app (`main.rs`)
- Commands added:
...
- Uses env override `CLIPDROP_DOWNLOAD_DIR` at runtime
...
- Main flow in `extension/content.js`:
  - Inject ClipDrop button on YouTube watch pages
...
- Added `/status/:job_id` polling fallback in content panel after queueing
